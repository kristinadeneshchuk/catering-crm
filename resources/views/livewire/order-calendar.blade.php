    ...
    <div class="fixed inset-0 z-50 flex items-center justify-center p-4" style="background: rgba(0,0,0,0.5);">
        <div class="bg-white dark:bg-gray-900 rounded-2xl shadow-2xl w-full max-w-lg flex flex-col" style="max-height: 90vh;" x-data="{ open: false }">
            ...
            <div class="px-6 py-5 space-y-4 overflow-y-auto flex-1 min-h-0">
                ...
                {{-- Календар показує дати ЇЖІ, а везуть раніше: при закритому
                     слоті недільний раціон їде ще в пʼятницю. Якщо доставка вже
                     була, видалення дня забирає гроші за те, що клієнт отримав. --}}
                @if($modalDeliveryPassed)
                    <div class="rounded-lg border border-red-300 dark:border-red-800 bg-red-50 dark:bg-red-900/20 px-3 py-2.5">
                        <p class="text-sm font-semibold text-red-700 dark:text-red-300">
                            Доставка вже відбулася — {{ $modalDeliveryDate }}
                        </p>
                        <p class="text-xs text-red-600 dark:text-red-400 mt-1">
                            Раціон на цей день клієнт уже отримав. Якщо видалити день,
                            з замовлення зникне і оплата за нього.
                        </p>
                    </div>
                @endif
                ...
                <div class="mb-5 border-t border-gray-200 dark:border-gray-700 pt-4">
                    ...
                    <div class="grid grid-cols-4 gap-2">
                        <div>
                            ...
                            <input type="number" min="0" step="1" wire:model="modalFakeKcal" placeholder="—"
                                class="w-full rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100 px-3 py-2 text-sm focus:ring-2 focus:ring-primary-500" />
                        </div>
                        <div>
                            <label class="block text-xs text-gray-500 mb-1">Білки</label>
                            <input type="number" min="0" step="0.1" wire:model="modalFakeProt" placeholder="—"
                                class="w-full rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100 px-3 py-2 text-sm focus:ring-2 focus:ring-primary-500" />
                        </div>
                        <div>
                            <label class="block text-xs text-gray-500 mb-1">Жири</label>
                            <input type="number" min="0" step="0.1" wire:model="modalFakeFat" placeholder="—"
                                class="w-full rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100 px-3 py-2 text-sm focus:ring-2 focus:ring-primary-500" />
                        </div>
                        <div>
                            <label class="block text-xs text-gray-500 mb-1">Вугл.</label>
                            <input type="number" min="0" step="0.1" wire:model="modalFakeCarb" placeholder="—"
                                class="w-full rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100 px-3 py-2 text-sm focus:ring-2 focus:ring-primary-500" />
                        </div>
                    </div>
                </div>
            ...
            <div class="px-6 py-4 border-t border-gray-200 dark:border-gray-700 flex items-center justify-between">
                <button type="button" wire:click="removeDay"
                    wire:confirm="{{ $modalDeliveryPassed
                        ? 'Доставка за цей день уже була ' . $modalDeliveryDate . '. Клієнт раціон отримав, а видалення прибере й оплату за нього. Точно видалити?'
                        : 'Видалити цей день з замовлення?' }}"
