    ...
    public function handle(AntLogisticsService $ant): int
    {
        ...
        $query->chunkById(500, function ($days) use (&$created, &$skipped, &$matched, $byAntId, $byNumDriver, $ant, $dry, $bar) {
            foreach ($days as $day) {
                ...
                //
                // Звіряємо по order_day_id — це та сама фізична доставка, а не
                // здогадка. У клієнта на одну дату цілком буває дві доставки,
                // ранкова і вечірня, ще й під тим самим номером маршруту
                // (в ANT нумерація починається заново кожну зміну). Ту, чия
                // шапка не збереглась, стирати не можна.
                if ($routeId) {
