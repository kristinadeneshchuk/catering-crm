...
trait CalculatesOrderPlan
{
    ...
    private function rebalanceForCustomMacros(array $itemsOut, Order $order, float $weightMultiplier): array
    {
        ...
        for ($iter = 0; $iter < 10; $iter++) {
            ...
            // Регуляризація: додаємо один рядок на кожну вільну змінну виду
            //   λ · g_i ≈ λ · baseW_i
            // Це:
            //   1) робить AᵀA завжди невиродженою (LS має єдиний розв'язок);
            //   2) при кількох валідних рішеннях під-детермінованої системи
            //      обирає найближче до стандартного грамажу — тобто «шевелимо
            //      порції тільки настільки, наскільки треба, щоб потрапити в цілі».
            // λ підібрано так, щоб макро-цілі домінували, але «якірна тяга»
            // до базових грамажів прибирала свободу під-детермінованої системи.
            $lambda = 0.05;
            foreach ($freeIdx as $k => $j) {
                $regRow = array_fill(0, count($freeIdx), 0.0);
                $regRow[$k] = $lambda;
                $A[] = $regRow;
                $b[] = $lambda * $baseW[$j];
            }
