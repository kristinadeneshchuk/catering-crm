...
{
    // 1. Отримуємо вибрану в пікері дату (день роботи/фасування)
    $dateParam = $this->data['date'] ?? now()->format('Y-m-d');
    
    // 2. 🔥 Визначаємо цільову дату (день споживання = завтра)
    // Замінюємо неіснуючу $selectedDate на визначену вище $dateParam
    ...
    return [
        Action::make('print_stickers')
            ...
            ->icon('heroicon-o-tag')
            ->color('gray')
            // 🔥 Використовуємо $targetDate, щоб стікери були на завтра
        ...
        Action::make('print_manifest')
            ...
            ->color('warning')
            // 🔥 Використовуємо $targetDate, щоб маніфест був на завтра
        ...
        Action::make('download_logistics')
            ...
            ->color('success')
            // 🔥 Використовуємо $targetDate для логістики на завтра
...
{
    // 1. Отримуємо вибрану в пікері дату (день фасування)
    $selectedDate = $this->data['date'] ?? now()->format('Y-m-d');
    
    // 2. 🔥 ГОЛОВНА ЛОГІКА: Фасуємо сьогодні на ЗАВТРА
    // Встановлюємо цільову дату споживання
    $targetDate = \Carbon\Carbon::parse($selectedDate)->addDay()->format('Y-m-d');
    
    $this->report = [];
    $this->debugMessage = null;

    // 3. Розраховуємо день циклу меню саме для ЦІЛЬОВОЇ дати (завтрашньої)
    ...
    if (!$menu) {
        $this->debugMessage = "⚠️ Меню на день споживання ({$targetDate}) не заповнено";
        return;
    }

    // 4. Шукаємо активні замовлення також на ЦІЛЬОВУ дату (завтра)
    $orders = Order::whereDate('start_date', '<=', $targetDate)
